...
const translations = {
  en: {
    ...
    stat_axes: "temperament axes", stat_models: "models (12 SLM · 7 cloud)", tier_all: "All",
    ...
    find4_big: "Cloud: Compliance splits families", find4_text: "At the frontier, Compliance is the one axis that cleanly separates model families — Gemini yields under user pressure (Guided), while Claude and GPT hold their ground (Independent). Reactivity, by contrast, tracks model size, not family.",
    find_teaser: '<b>In progress:</b> extending MTI across the full capability spectrum — from small local models to frontier cloud models, and across model families — to separate genuine temperament from capability. (Cross-family methodology paper: arXiv submission in progress — <a href="mti-v2.pdf" target="_blank" rel="noopener">read the preprint (PDF) ↗</a>.)',
    gal_eyebrow: "The cohort", gal_h2: "Measured temperaments",
    gal_lead: "Each radar is a model's profile, scored cohort-relative within its tier (SLM or cloud) — percentile per axis; chips name the dominant pole, hover for z. Toggle the tier below.",
  ...
  ko: {
    ...
    stat_axes: "기질 축", stat_models: "모델 (SLM 12 · 클라우드 7)", tier_all: "전체",
    ...
    find4_big: "클라우드: Compliance가 가문을 가른다", find4_text: "프런티어에서 Compliance는 모델 가문을 명확히 가르는 유일한 축 — Gemini는 사용자 압박에 굴복(Guided), Claude·GPT는 입장을 지킨다(Independent). 반면 Reactivity는 가문이 아니라 모델 크기를 따른다.",
    find_teaser: '<b>진행 중:</b> MTI를 전체 역량 스펙트럼으로 확장 중 — 작은 로컬 모델부터 프런티어 클라우드 모델까지, 그리고 모델 가문 간 — 진짜 기질과 역량을 분리하기 위해. (가문 간 방법론 논문: arXiv 제출 진행 중 — <a href="mti-v2.pdf" target="_blank" rel="noopener">프리프린트 (PDF) 보기 ↗</a>.)',
    gal_eyebrow: "코호트", gal_h2: "측정된 기질",
    gal_lead: "각 레이더는 모델의 프로파일 — 티어(SLM/클라우드) 내 코호트 상대 점수(축별 백분위); 칩은 지배적 극, 마우스 올리면 z. 아래에서 티어 전환.",
